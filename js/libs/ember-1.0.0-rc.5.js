...
define("metamorph",
  ...
  function() {
    ...
    var K = function(){},
        ...
        // turn off because https://github.com/emberjs/ember.js/pull/3110
        supportsRange = false && document && ('createRange' in document) && (typeof Range !== 'undefined') && Range.prototype.createContextualFragment,
...
Ember.Select = Ember.View.extend(
  ...
  defaultTemplate: Ember.Handlebars.template(function anonymous(Handlebars, depth0, helpers, partials, data) { this.compilerInfo = [3,'>= 1.0.0-rc.4'];
  ...
  function(RouteRecognizer, RSVP) {
    ...
    */
    ...
    Transition.prototype = {
      ...
      retry: function() {
        ...
        var recogHandlers = this.router.recognizer.handlersFor(this.targetName),
            newTransition = performTransition(this.router, recogHandlers, this.providedModelsArray, this.params);

        newTransition.data = this.data;
    ...
    function getMatchPoint(router, handlers, objects, inputParams) {
      ...
      return { matchPoint: matchPoint, providedModels: providedModels, params: params };
    }
    ...
    */
    ...
    function handlerEnteredOrUpdated(transition, currentHandlerInfos, handlerInfo, enter) {
      ...
      try {
        ...
        if (!(e instanceof Router.TransitionAborted) && handler.error) { handler.error(e); }
    ...
    */
    ...
    function trigger(router, args) {
      ...
      if (!eventWasHandled && name !== 'willTransition') {
     ...
     */
    function performTransition(router, recogHandlers, providedModelsArray, params) {
      ...
      validateEntry(transition, handlerInfos, 0, matchPointResults.matchPoint)
    ...
    function validateEntry(transition, handlerInfos, index, matchPoint) {
      ...
      if (index < matchPoint) {
        ...
        //return RSVP.resolve(handlerInfo.handler.context).then(proceed);
        return proceed(handlerInfo.handler.context);
      }

      return RSVP.resolve().then(beforeModel);

      function handleError(reason) {
        log(router, seq, handlerName + ": handling error: " + reason);

        // An error was thrown / promise rejected, so run
        // the default `error` handler if it exists.
        if (handler.error) { handler.error(reason, transition); }
      ...
      function beforeModel() {
        ...
        return RSVP.resolve(handler.beforeModel && handler.beforeModel(transition))
      ...
      function model() {
        ...
        var newModel = getModel(handlerInfo, transition, index >= matchPoint);

        return RSVP.resolve(newModel).then(null, handleError).then(afterModel);
      }
      ...
      function afterModel(context) {
        ...
        return RSVP.resolve(handler.afterModel && handler.afterModel(context, transition))
                   .then(null, handleError)
                   .then(function() { return proceed(context); });
      }
      ...
      function proceed(context) {
        ...
        // Save the resolved context into the handlerInfo that
        // we'll be passing to setupContexts. Also save it into the
        // resolvedModels hash so that it's available to leafier
        // handlers while they're being validated.

        //merge(transition.params, serialize(handler, context, names));

        handlerInfo.context = transition.resolvedModels[handlerInfo.name] = context;
    ...
    function getModel(handlerInfo, transition, needsUpdate) {
      ...
      return handler.model && handler.model(transition.params, transition);
    }
